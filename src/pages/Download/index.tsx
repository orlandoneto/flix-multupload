...
import { ActionButtons, AreaInfo, AreaTags, AreaUserProfile, CategoryBadge, FeatureList, FollowButton, LeftSection, RightSection, TagsSection, Title, UserProfile } from '~/components/Modals/ModalDownload/styles';
import { SEO } from '~/components/SEO';
import { Routes } from '~/routes/routes';
import { ComplaintsService, PlanDownloadLimitService, S3DownloaService, SubscriptionService, UseDownloadService, UserBugService, UserFavoritesService, UserFollowsService } from '~/services';
import { theme } from '~/theme';
import { downloadFile, extractKeyFromUrl, isImageUrl } from '~/utils';
import { useToast, useUserData, useUserDataCache } from '~/utils/hook';
...
const DownloadPage: React.FC = () => {
    ...
    const existPlan = planUser && (planUser as UserPlan)?.plan_id;
    ...
    const getDownloadsByUser = async (userId: number): Promise<UserPlanDownloads | null> => {
        try {
            const response = await subscriptionService.getUserPlanDownloads(userId);
            return response.data as UserPlanDownloads;
        } catch (error) {
        ...
        if (isLoggedParam && existPlan) {
            ...
            if (downloadCount && limitDownloads) {
                ...
                if (downloadCount.count_downloads === actualLimit.current_count_downloads) {
                    alert('Você usou o limite de downloads diários!');
                    return;
                } else if (downloadCount.count_downloads > actualLimit.current_count_downloads) {
                    await planDownloadLimitService.update(planUserParam.user_id);
                    if (itemParam) handleDownload(itemParam.url, itemParam.name);
                }
            }
        } else {
    ...
    async function shouldResetDailyLimit(updated_at: string) {
        // Reset ao virar o dia: considera desbloqueado a partir da meia-noite posterior ao último update
        const now = new Date();
    ...
    return (
        <>
            <SEO
                ...
                url={typeof window !== 'undefined' ? window.location.pathname : `/download/${item.id}`}
                type="product"
            />
            <Header />
            <Body>
                                ...
                                <>
                                    <LeftSection style={{ height: 'auto', width: '100%', maxWidth: '100%', position: 'relative' }}>
                                        ...
                                        <div style={{ position: 'relative' }}>
                                            ...
                                            <img src={item.url_cover} alt={item.name} style={{ width: '100%', display: 'block', borderRadius: 0 }} />
                                        </div>
                                    </LeftSection>
                                    ...
                                    <RightSection>
                                        ...
                                        <FacadeButton
                                            ...
                                            typeClick="download"
                                            handleDownload={() => handleCheckPermissionDownloadImage(isLogged, item, planUser as UserPlan)}
                                ...
                                <>
                                    <LeftSection>
                                        <div style={{ position: 'relative' }}>
                                            ...
                                            <img src={item.url_cover} alt={item.name} />
                                        </div>
                                    </LeftSection>
                                    <RightSection>
                                        <AreaTags>
            ...
            >
                {item && (
                    <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
                        <WhatsappShareButton url={`${window.location.origin}/download/${item.id}`} title={`Veja ${item.name} no Designflix`}>
                            <WhatsappIcon size={32} round />
                        </WhatsappShareButton>
                        <FacebookShareButton url={`${window.location.origin}/download/${item.id}`} quote={item.name}>
                            <FacebookIcon size={32} round />
                        </FacebookShareButton>
                        <PinterestShareButton url={`${window.location.origin}/download/${item.id}`} media={item.url_cover} description={item.name}>
                            <PinterestIcon size={32} round />
                        </PinterestShareButton>
                        <LinkedinShareButton url={`${window.location.origin}/download/${item.id}`} title={item.name} summary={item.name}>
                            <LinkedinIcon size={32} round />
                        </LinkedinShareButton>
                        <TwitterShareButton url={`${window.location.origin}/download/${item.id}`} title={item.name}>
                            <TwitterIcon size={32} round />
                        </TwitterShareButton>
                        <TelegramShareButton url={`${window.location.origin}/download/${item.id}`} title={item.name}>
                            <TelegramIcon size={32} round />
                        </TelegramShareButton>
                        <EmailShareButton url={`${window.location.origin}/download/${item.id}`} subject={item.name} body={`Veja ${item.name}: ${window.location.origin}/download/${item.id}`}>
